...
1. **Live SQL Database Auto-Fetch**: Automatically query the live `wms` MySQL database when a **`TRAY ID`** is scanned or entered, performing real-time indexed queries across `wms.tray_monitoring`, `wms.order_items`, `wms.fitting_detail`, and `wms.power` to auto-populate `FITTING ID`, `ORDER ID`, `REQ. FRAME PID`, `REQ. RIGHT LENS BARCODE`, `REQ. LEFT LENS BARCODE`, `LENS INDEX` (`1.56`, `1.60`, etc.), and **SEPARATE Optical Powers for BOTH Right Lens & Left Lens** (`SPH`, `CYL`, `AXIS`, `ADDN`) in under 10 milliseconds.
